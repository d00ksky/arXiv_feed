...
def get_limited_titles_after_year(
    papers: list[Paper], year: int, limit: int
) -> list[str]:
...
def top_authors(papers: list[Paper], n: int = 5) -> list[tuple[str, int]]:
    ...
    authors_sorted = sorted(
        authors_counts.items(), key=lambda author: author[1], reverse=True
    )
...
def newest_paper(papers: list[Paper]) -> Paper | None:
    ...
    return max(papers, key=lambda x: x.year, default=None)
...
def papers_by_author_sorted(papers: list[Paper], author: str) -> list[Paper]:
    ...
    return sorted(
        papers_by_author(papers, author), key=lambda paper: paper.year, reverse=True
    )
...
def top_papers_by_year(papers: list[Paper], year: int) -> list[str]:
    ...
    sorted_papers = sorted(
        papers_from_year, key=lambda paper: paper.citations, reverse=True
    )
...
def search_and_rank(papers: list[Paper], keyword: str, n: int) -> list[Paper]:
...
def recent_papers(papers: list[Paper], n: int = 5) -> list[Paper]:
    ...
    """
    return sorted(
        papers, key=lambda paper: (paper.year, paper.citations), reverse=True
    )[:n]
